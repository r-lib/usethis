...
We start the process of contributing to the package with `pr_init()`, which creates a branch in our repository for the pull request. It is a good idea to make your pull requests from a non-`master` branch. We'll call this branch `"formidable"`.
...
Let's go ahead and make the change, which is adding the word "formidable" to the `adjective.R` file in the package. Below is the diff and the commit associated with this change.
...
This will first prompt the following:

<div class = "output">
```{r eval=FALSE}
#> Which repo should we target? 

#> 1: mine-cetinkaya-rundel/praise = 'origin'
#> 2: rladies/praise = 'upstream'
```
</div>

If you choose 2, it will output the following and open the PR in your browser.
...
This will again prompt the following:
...
If you choose 1, it will finish up the PR and delete your local `formidable` branch.
