...
All of this output can be turned off package-wide via the `usethis.quiet` option.
...
The overall conversion plan is to switch to the new `ui_bullets()`, which is a wrapper around `cli::cli_bullets()`, that adds a few features:
...
(I have renamed `ui_bullet()` to `ui_legacy_bullet()` for auto-completion happiness with the new `ui_bullets()`.)
...
```
It would be nice to create a custom inline class, e.g. `{.usethis_path {some_path}}`, which I have done in, e.g., googledrive. But it's not easy to do this *while still inheriting cli's `file:` hyperlink behaviour*, which is very desirable. So that leads to the somewhat clunky pattern above, but it gives a nice result.
...
Inline styles that don't get quoted in the absence of color and that I'm worried about:

* ~`.field`~ I've fixed this in the theme
* `.pkg`
...
I also use `ui_abort()` to apply different default bullet styling.
...
```{r}
ui_abort(c("v" = "Weird thing to do in an error, but whatever."))
ui_abort(c(
...
Now that I'm looking at a lot of the new errors with `ui_abort()` I realize that usethis also needs to be passing the `call` argument along.
I'm not going to tackle this in the main conversion.
...
TO RECONSIDER: If I keep using `ui_yeah()` and `ui_nope()`, that means I retain usage of legacy inline styling. For example:
...
If I create new versions of `ui_yeah()` and `ui_nope()`, then I could still switch over to cli's inline styling. Otherwise, I won't be able to remove all internal usage of legacy `ui_*()` functions.
...
```
  ...
  - Here's another style that applies to `ui_abort()`, where there's just one, unnamed bullet, but the call doesn't fit on one line.
```{r}
...
TODO: come back to this bit in git.R: ` purrr::walk(format(cfg), ui_bullet)`
