...
* `indent()` turns out to still be useful in `ui_code_snippet()`.
...
`up_path()` first makes the path relative to a specific base (by default, the active project root) and, if the path is a directory, it also ensures there is a trailing `/`.
...
So that leads to the somewhat clunky pattern above, but it gives a nice result.
-   `ui_code()` gets replaced by various inline styles, depending on what the actual goal is, such as:
    ...
    -   `{.topic some_topic}`
    -   `{.var some_variable}`
- `ui_unset()` is replaced by `ui_special()`, which you'll see more of below.
...
It has very little usage and, instead of converting it, I've eliminated its use altogether in favor of `ui_bullets(c("!" = "Thing I'm warning about."))`.
...
* Interpolates data and applies inline style to the result.
  Works differently for `key` and `value`.
...
noun <- "thingy"
value <- "VALUE"
kv_line("Key associated with {.val {noun}}", value)

kv_line("URL for the {.val {remote}} remote", I("{.url {url}})"))

kv_line("Personal access token for {ui_value(host)}", NULL)

kv_line("Personal access token for {ui_value(host)}", ui_special("discovered"))
```

`ui_unset()` is replaced by `ui_special()`.
