...
At this point, the `usethis_theme()` doesn't do anything that is perceivable in an error.
So the main point of `ui_abort()` is to switch to `cli_abort()`.

Say something about `ui_warn()` once I get there:
