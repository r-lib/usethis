...
Certain options are consulted by usethis and allow you to set personal defaults:
  ...
  * `usethis.destdir`: a default directory to use in `create_from_github()` and
    `use_course()`.
  * `usethis.overwrite`: if `TRUE` and you're using Git in the current project,
    usethis overwrites existing files without asking. You will see and can selectively accept/discard any proposed changes as part of the normal Git workflow.
