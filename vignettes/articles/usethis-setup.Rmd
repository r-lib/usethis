...
You will get the most out of usethis if you do some setup, although none of this is required. usethis can even help you with these setup tasks!
...
For instructions on how to install Git, please see [Happy Git and GitHub for the useR](http://happygitwithr.com/install-git.html). It is beyond the scope of this article.

Once Git is installed, you must introduce yourself to Git.

```{r, eval = FALSE}
library(usethis)
use_git_config(user.name = "Jenny", user.email = "jenny@example.com")

## confirm the config
use_git_config()
```

`usethis::use_git_config()` helps you configure your global or user-level `user.name` and `user.email`. Substitute **your name** and **your email address**.
...
git config --global user.email 'jenny@example.com'
...
Substitute your preferred editor for emacs here. A popular choice is nano.
