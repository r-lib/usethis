...
`create_github_token()` provides advice on how to store your PAT, also repeated here. It is customary to save the PAT as an environment variable in your `.Renviron`, with the name `GITHUB_PAT`.
...
A more secure method of storage is to store your PAT in the Git credential store used by your operating system, not in plain text in `.Renviron`. How to set that up?

```{r, eval = FALSE}
# initial, interactive call to store the PAT
credentials::set_github_pat()
```

Your first call to `credentials::set_github_pat()` will prompt you for your PAT and will store it. Subsequent calls retrieve this value and assign it to the `GITHUB_PAT` environment variable. Therefore, you might want to put this in your `.Rprofile`:
...
Remember `edit_r_profile()` helps you get this file open for editing.

The GitHub section of `usethis::git_sitrep()` reveals if a PAT is found and, if so, the associated user.
