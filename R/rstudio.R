...
#' Use AGG as RStudio's Graphic Backend
#'
#' `use_ragg()` sets the user's default graphic device in RStudio to use AGG
#' (via the ragg R package). Compared to the default graphics device, AGG is
#' faster, higher quality, and more consistent across operating systems. More
#' detail can be found in the blog post [Modern Text Features in
#' R](https://www.tidyverse.org/blog/2021/02/modern-text-features/). Setting the
#' backend graphics device only affects plots generated by RStudio; to use ragg
#' in R Markdown, set `dev = "ragg_png"` in your chunk options.
#'
#'
#' @export
use_ragg <- function() {
  check_installed("ragg")
  use_rstudio_config(list(graphics_backend = "ragg"))
}
